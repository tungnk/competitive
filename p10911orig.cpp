 ...
 * =====================================================================================
 *
 *       Filename:  p10911orig.cpp
 *
 *    Description:
 *
 *        Version:  1.0
 *        Created:  06/25/2023 01:34:01
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  Nguyen khac Tung (), Nguyenkhactungvn@gmail.com
...
{ // DP state = bitmask
  // we initialize ‘memo’ with -1 in the main function
	...
	int p1, p2;
	for (p1 = 0; p1 < 2 * N; p1++)// find the first bit that is off 
		if (!(bitmask & (1 << p1)))
			break;
	...
	return memo[bitmask] = ans; // store result in memo table and return
}
...
int main()
{
	...
	// redirect input file to stdin // yes, we can do this :)
	while (scanf("%d", &N), N)
	{
		for (i = 0; i < 2 * N; i++)
			scanf("%*s %d %d", &x[i], &y[i]);  // %s skip name
	
		for (i = 0; i < 2 * N - 1; i++) // build pairwise distance table 
			for (j = i + 1; j < 2 * N; j++) // have you used ‘hypot’ before?
				dist[i][j] = dist[j][i] = hypot(x[i] - x[j], y[i] - y[j]);
		// with another bit matching(bitmask | (1 <<  p2 that is also off 
		// pick the minimum p1) | (1 << p2)));
	
		// use DP to solve min weighted perfect matching on small general graph
	for (i = 0; i < (1 << 16); i++)
		memo[i] = -1.0; // set-1toallcells
	target = (1 << (2 * N)) - 1;
	printf("Case %d: %.2lf\n", caseNo++, matching(0));
}
} // return 0;
